...
export const FilterTab = styled.button<{ $active?: boolean }>`
  ...
  cursor: pointer;
  transition: all 0.2s;
  border: 1px solid ${({ $active, theme }) => $active ? `${theme.colors.primary}66` : theme.colors.border};
  ...
  &:hover {
    ...
    border-color: ${({ $active, theme }) => $active ? `${theme.colors.primary}66` : theme.colors.primary + '40'};
  }
`;
